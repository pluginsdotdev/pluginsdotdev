...
const applyUpdates = (
  ...
  updates.reduce((rootNode: RootNode, update: ReconciliationUpdate) => {
    ...
    if (typeof update.propUpdates !== "undefined") {
      const propsToSet = update.propUpdates.filter(
        (u): u is ReconciliationSetPropUpdate => u.op === "set"
      );
      const propsToDel = update.propUpdates.filter(
        (u): u is ReconciliationDeletePropUpdate => u.op === "delete"
      );
    ...
    if (typeof update.childUpdates !== "undefined") {
      const childrenToSet = update.childUpdates.filter(
        (c): c is ReconciliationSetChildUpdate => c.op === "set"
      );
      const childrenToDel = new Set(
        update.childUpdates
          .filter(
            (c): c is ReconciliationDeleteChildUpdate => c.op === "delete"
          )
          .map((c) => c.childId)
      );
      const withoutDeleted = node.children.filter(
        childrenToDel.has.bind(childrenToDel)
      );
      const children = childrenToSet.reduce(
        (children, { childId, childIdx }) => {
